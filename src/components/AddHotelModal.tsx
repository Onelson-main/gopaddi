"use client"
import { searchHotels } from "@/lib/api/hotel_service"
import { useStore } from "@/lib/store"
import { HotelDetail } from "@/lib/types/hotel_types"
import { Formik } from "formik"
import Image from "next/image"
import Link from "next/link"
import { useState } from "react"
import { toast } from "react-toastify"

const cities = [
    { name: "Lagos", latitude: 6.5244, longitude: 3.3792 },
    { name: "Abuja", latitude: 9.0765, longitude: 7.3986 },
    { name: "Port Harcourt", latitude: 4.8156, longitude: 7.0498 },
    { name: "Kano", latitude: 12.0022, longitude: 8.5920 },
    { name: "London", latitude: 51.5074, longitude: -0.1278 },
    { name: "New York", latitude: 40.7128, longitude: -74.0060 },
    { name: "New Delhi", latitude: 28.6139, longitude: 77.2090 },
    { name: "Dubai", latitude: 25.2048, longitude: 55.2708 },
    { name: "Paris", latitude: 48.8566, longitude: 2.3522 },
    { name: "Amsterdam", latitude: 52.3676, longitude: 4.9041 },
    { name: "Frankfurt", latitude: 50.1109, longitude: 8.6821 },
]

interface CityDropdownProps {
    isOpen: boolean
    searchTerm: string
    onSearchChange: (value: string) => void
    onSelect: (city: { name: string; latitude: number; longitude: number }) => void
    onClose: () => void
}

function CityDropdown({ isOpen, searchTerm, onSearchChange, onSelect, onClose }: CityDropdownProps) {
    if (!isOpen) return null

    const filteredCities = cities.filter(
        (city) => city.name.toLowerCase().includes(searchTerm.toLowerCase())
    )

    return (
        <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-b shadow-lg z-10 max-h-60 overflow-y-auto">
            <div className="p-2 border-b">
                <input
                    type="text"
                    placeholder="Search cities..."
                    value={searchTerm}
                    onChange={(e) => onSearchChange(e.target.value)}
                    className="w-full p-2 border rounded text-sm"
                    autoFocus
                />
            </div>
            <div className="max-h-48 overflow-y-auto">
                {filteredCities.map((city) => (
                    <div
                        key={city.name}
                        className="p-3 hover:bg-gray-100 cursor-pointer border-b last:border-b-0"
                        onClick={() => {
                            onSelect(city)
                            onClose()
                        }}
                    >
                        <span className="font-medium">{city.name}</span>
                    </div>
                ))}
                {filteredCities.length === 0 && (
                    <div className="p-3 text-gray-500 text-center">No cities found</div>
                )}
            </div>
        </div>
    )
}

export default function AddHotelModal() {
    const [isDropdownOpen, setIsDropdownOpen] = useState(false)
    const [searchTerm, setSearchTerm] = useState('')
    const [hotels, setHotels] = useState<HotelDetail[]>([])

    const { addHotel, showLoading, hideLoading } = useStore()


    const today = new Date().toISOString().split('T')[0]

    return (
        <div className="fixed z-[100] left-0 top-0 bg-[#000000E6] h-full w-full flex justify-center items-center p-10">
            <div className="relative w-fit max-w-[1200px] bg-white rounded max-h-90p flex flex-col">
                <Link href={"/"} className="absolute right-1 top-1">
                    <svg
                        className="h-6 w-6 text-NEUTRAL"
                        fill="currentColor"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 512 512"
                    >
                        <path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm75.31 260.69a16 16 0 11-22.62 22.62L256 278.63l-52.69 52.68a16 16 0 01-22.62-22.62L233.37 256l-52.68-52.69a16 16 0 0122.62-22.62L256 233.37l52.69-52.68a16 16 0 0122.62 22.62L278.63 256z" />
                    </svg>
                </Link>

                <Formik
                    initialValues={{
                        selectedCity: null as { name: string; latitude: number; longitude: number } | null,
                        checkInDate: "",
                        checkOutDate: ""
                    }}
                    onSubmit={async (values) => {
                        showLoading()
                        try {
                            if (!values.selectedCity) {
                                toast.info("Please fill the required details")
                                return;
                            }
                            // @ts-expect-error null
                            const res = await searchHotels(values)
                            if (!res.status) {
                                toast.error("No Flights found")
                            }
                            console.log(res.data);
                            setHotels(res.data)
                        } catch (err) {
                            console.log(err);
                            toast.error("an error occurred")
                        }
                        hideLoading()
                    }}
                >
                    {({ values, handleSubmit, setFieldValue }) => (
                        <div className="bg-white p-4 w-full rounded flex flex-col">
                            <strong><h1 className="text-2xl mb-2">Search Hotels</h1></strong>

                            <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                                <div className="relative w-52.5 flex flex-col p-4 rounded bg-white hover:bg-gray-100 cursor-pointer">
                                    <span className="text-xs text-gray-600">DESTINATION</span>
                                    <div
                                        className="text-xl font-bold whitespace-nowrap"
                                        onClick={() => {
                                            setIsDropdownOpen(!isDropdownOpen)
                                            setSearchTerm("")
                                        }}
                                    >
                                        {values.selectedCity ? values.selectedCity.name : "Select City"}
                                    </div>
                                    <CityDropdown
                                        isOpen={isDropdownOpen}
                                        searchTerm={searchTerm}
                                        onSearchChange={setSearchTerm}
                                        onSelect={(val) => { setFieldValue("selectedCity", val); setSearchTerm('') }}
                                        onClose={() => setIsDropdownOpen(false)}
                                    />
                                </div>

                                {/* Check-in Date */}
                                <div className="flex w-52.5 flex-col p-4 rounded bg-white hover:bg-gray-100 cursor-pointer">
                                    <span className="text-xs text-gray-600">CHECK-IN DATE</span>
                                    <input
                                        type="date"
                                        className="text-xl font-bold bg-transparent"
                                        value={values.checkInDate}
                                        onChange={(e) => setFieldValue("checkInDate", e.target.value)}
                                        min={today}
                                    />
                                </div>

                                {/* Check-out Date */}
                                <div className="flex w-52.5 flex-col p-4 rounded bg-white hover:bg-gray-100 cursor-pointer">
                                    <span className="text-xs text-gray-600">CHECK-OUT DATE</span>
                                    <input
                                        type="date"
                                        className="text-xl font-bold bg-transparent"
                                        value={values.checkOutDate}
                                        onChange={(e) => setFieldValue("checkOutDate", e.target.value)}
                                        min={values.checkInDate || today}
                                    />
                                </div>
                            </div>

                            {/* Search Button */}
                            <div className="bg-gray-50 p-2 w-fit self-end rounded-b">
                                <button
                                    className="left-auto bg-blue-600 hover:bg-blue-700 text-white font-bold p-4 rounded"
                                    onClick={() => handleSubmit()}
                                    type="button"
                                >
                                    SEARCH HOTELS
                                </button>
                            </div>
                        </div>
                    )}
                </Formik>
                {hotels.length > 0 && <div className="max-h-[60vh] overflow-y-scroll bg-BACKGROUND flex flex-col gap-8 rounded-t p-8">
                    {hotels.map((hotel, index) => (
                        <div key={index} className='flex items-stretch w-full bg-BACKGROUND-2 rounded'>
                            <div className="flex-1 rounded-xl overflow-hidden flex flex-col p-4 space-y-4 pr-0">
                                <div className="flex flex-col md:flex-row">
                                    <div
                                        className="relative min-w-[150px] max-w-[200px] rounded-lg overflow-hidden"
                                        style={{ backgroundImage: `url("${hotel.imageUrl}")` }}>
                                        <Image
                                            height={200}
                                            width={200}
                                            src={hotel.imageUrl}
                                            alt="Hotel image"
                                            className="object-cover rounded aspect-square bg-ACCENT"
                                            quality={100}
                                        />
                                        <button className="absolute left-2 top-1/2 transform -translate-y-1/2 p-1">
                                            <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M13 26C15.5712 26 18.0846 25.2376 20.2224 23.8091C22.3602 22.3806 24.0265 20.3503 25.0104 17.9749C25.9944 15.5994 26.2518 12.9856 25.7502 10.4638C25.2486 7.94207 24.0105 5.62569 22.1924 3.80761C20.3743 1.98953 18.0579 0.7514 15.5362 0.249792C13.0144 -0.251816 10.4005 0.00562668 8.02511 0.989567C5.64967 1.97351 3.61935 3.63975 2.19089 5.77759C0.76243 7.91543 -5.72205e-06 10.4288 -5.72205e-06 13C0.0036335 16.4467 1.37444 19.7512 3.81163 22.1884C6.24881 24.6256 9.5533 25.9964 13 26ZM9.29249 12.2925L14.2925 7.2925C14.3854 7.19959 14.4957 7.12589 14.6171 7.07561C14.7385 7.02532 14.8686 6.99944 15 6.99944C15.1314 6.99944 15.2615 7.02532 15.3829 7.07561C15.5043 7.12589 15.6146 7.19959 15.7075 7.2925C15.8004 7.38541 15.8741 7.49571 15.9244 7.6171C15.9747 7.7385 16.0006 7.8686 16.0006 8C16.0006 8.1314 15.9747 8.2615 15.9244 8.3829C15.8741 8.50429 15.8004 8.61459 15.7075 8.7075L11.4137 13L15.7075 17.2925C15.8951 17.4801 16.0006 17.7346 16.0006 18C16.0006 18.2654 15.8951 18.5199 15.7075 18.7075C15.5199 18.8951 15.2654 19.0006 15 19.0006C14.7346 19.0006 14.4801 18.8951 14.2925 18.7075L9.29249 13.7075C9.19952 13.6146 9.12576 13.5043 9.07543 13.3829C9.02511 13.2615 8.99921 13.1314 8.99921 13C8.99921 12.8686 9.02511 12.7385 9.07543 12.6171C9.12576 12.4957 9.19952 12.3854 9.29249 12.2925Z" fill="#F9FAFB" />
                                            </svg>
                                        </button>

                                        <button className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1">
                                            <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M13 0C10.4288 0 7.91543 0.762437 5.77759 2.1909C3.63975 3.61935 1.97351 5.64968 0.989572 8.02512C0.0056327 10.4006 -0.251811 13.0144 0.249797 15.5362C0.751405 18.0579 1.98953 20.3743 3.80762 22.1924C5.6257 24.0105 7.94208 25.2486 10.4638 25.7502C12.9856 26.2518 15.5995 25.9944 17.9749 25.0104C20.3503 24.0265 22.3807 22.3603 23.8091 20.2224C25.2376 18.0846 26 15.5712 26 13C25.9964 9.5533 24.6256 6.24882 22.1884 3.81163C19.7512 1.37445 16.4467 0.00363977 13 0ZM16.7075 13.7075L11.7075 18.7075C11.6146 18.8004 11.5043 18.8741 11.3829 18.9244C11.2615 18.9747 11.1314 19.0006 11 19.0006C10.8686 19.0006 10.7385 18.9747 10.6171 18.9244C10.4957 18.8741 10.3854 18.8004 10.2925 18.7075C10.1996 18.6146 10.1259 18.5043 10.0756 18.3829C10.0253 18.2615 9.99945 18.1314 9.99945 18C9.99945 17.8686 10.0253 17.7385 10.0756 17.6171C10.1259 17.4957 10.1996 17.3854 10.2925 17.2925L14.5863 13L10.2925 8.7075C10.1049 8.51986 9.99945 8.26536 9.99945 8C9.99945 7.73464 10.1049 7.48014 10.2925 7.2925C10.4801 7.10486 10.7346 6.99944 11 6.99944C11.2654 6.99944 11.5199 7.10486 11.7075 7.2925L16.7075 12.2925C16.8005 12.3854 16.8742 12.4957 16.9246 12.6171C16.9749 12.7385 17.0008 12.8686 17.0008 13C17.0008 13.1314 16.9749 13.2615 16.9246 13.3829C16.8742 13.5043 16.8005 13.6146 16.7075 13.7075Z" fill="#F9FAFB" />
                                            </svg>
                                        </button>
                                    </div>

                                    {/* Hotel Info */}
                                    <div className="flex-1 p-4">
                                        <div className="flex justify-between">
                                            <div>
                                                <h3 className="text-lg font-semibold">{hotel.name}</h3>
                                                <p className="font-medium">{hotel.address}</p>
                                                <div className="flex items-center space-x-4 mt-1 text-sm text-gray-600">
                                                    <span className="text-ACCENT font-medium flex items-center gap-2 cursor-pointer hover:underline">
                                                        <svg width="14" height="18" viewBox="0 0 14 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M7 4.21875C6.38811 4.21875 5.78997 4.4002 5.2812 4.74014C4.77244 5.08009 4.37591 5.56326 4.14175 6.12857C3.90759 6.69388 3.84632 7.31593 3.9657 7.91606C4.08507 8.51619 4.37972 9.06744 4.81239 9.50011C5.24506 9.93278 5.79631 10.2274 6.39644 10.3468C6.99657 10.4662 7.61862 10.4049 8.18393 10.1708C8.74924 9.93659 9.23241 9.54006 9.57236 9.03129C9.9123 8.52253 10.0938 7.92439 10.0938 7.3125C10.0928 6.49227 9.76657 5.7059 9.18658 5.12592C8.6066 4.54593 7.82023 4.21968 7 4.21875ZM7 8.71875C6.72187 8.71875 6.44999 8.63627 6.21873 8.48175C5.98747 8.32723 5.80723 8.10761 5.70079 7.85065C5.59436 7.59369 5.56651 7.31094 5.62077 7.03815C5.67503 6.76537 5.80896 6.5148 6.00563 6.31813C6.2023 6.12146 6.45287 5.98753 6.72565 5.93327C6.99844 5.87901 7.28119 5.90686 7.53815 6.01329C7.79511 6.11973 8.01473 6.29997 8.16925 6.53123C8.32377 6.76249 8.40625 7.03437 8.40625 7.3125C8.40625 7.68546 8.25809 8.04315 7.99437 8.30687C7.73065 8.57059 7.37296 8.71875 7 8.71875ZM7 0.84375C5.28495 0.845611 3.64068 1.52774 2.42796 2.74046C1.21524 3.95318 0.533111 5.59745 0.53125 7.3125C0.53125 12.7519 6.27156 16.8328 6.51625 17.0037C6.65805 17.1029 6.82693 17.1561 7 17.1561C7.17307 17.1561 7.34195 17.1029 7.48375 17.0037C8.57097 16.2024 9.56568 15.2827 10.4495 14.2615C12.4246 11.9932 13.4688 9.58852 13.4688 7.3125C13.4669 5.59745 12.7848 3.95318 11.572 2.74046C10.3593 1.52774 8.71505 0.845611 7 0.84375ZM9.20078 13.128C8.53038 13.8981 7.79398 14.6082 7 15.2501C6.20602 14.6082 5.46962 13.8981 4.79922 13.128C3.625 11.7682 2.21875 9.65461 2.21875 7.3125C2.21875 6.04443 2.72249 4.8283 3.61915 3.93165C4.5158 3.03499 5.73193 2.53125 7 2.53125C8.26807 2.53125 9.4842 3.03499 10.3809 3.93165C11.2775 4.8283 11.7812 6.04443 11.7812 7.3125C11.7812 9.65461 10.375 11.7682 9.20078 13.128Z" fill="#0D6EFD" />
                                                        </svg>
                                                        {hotel.showInMap ? "Show in map" : ""}
                                                    </span>
                                                    <span className="flex items-center gap-2.5">
                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M15.4884 7.0422L12.3173 9.8097L13.2672 13.93C13.3175 14.1454 13.3031 14.3708 13.226 14.5781C13.1488 14.7854 13.0123 14.9654 12.8334 15.0955C12.6545 15.2256 12.4413 15.3001 12.2203 15.3096C11.9993 15.3192 11.7804 15.2634 11.591 15.1492L7.9973 12.9695L4.41136 15.1492C4.22192 15.2634 4.00303 15.3192 3.78205 15.3096C3.56107 15.3001 3.34781 15.2256 3.16894 15.0955C2.99006 14.9654 2.85351 14.7854 2.77635 14.5781C2.69919 14.3708 2.68485 14.1454 2.73511 13.93L3.68363 9.81392L0.511832 7.0422C0.344073 6.89752 0.222765 6.70652 0.163121 6.49317C0.103478 6.27981 0.108154 6.0536 0.176562 5.84289C0.244971 5.63218 0.374067 5.44636 0.547662 5.30873C0.721257 5.1711 0.931623 5.08779 1.15238 5.06923L5.33316 4.70712L6.96511 0.814625C7.05034 0.610379 7.19409 0.435913 7.37826 0.313197C7.56243 0.190481 7.7788 0.125 8.00011 0.125C8.22143 0.125 8.43779 0.190481 8.62197 0.313197C8.80614 0.435913 8.94989 0.610379 9.03511 0.814625L10.672 4.70712L14.8514 5.06923C15.0721 5.08779 15.2825 5.1711 15.4561 5.30873C15.6297 5.44636 15.7588 5.63218 15.8272 5.84289C15.8956 6.0536 15.9003 6.27981 15.8406 6.49317C15.781 6.70652 15.6597 6.89752 15.4919 7.0422H15.4884Z" fill="#F4B93E" />
                                                        </svg>
                                                        {hotel.rating}
                                                    </span>
                                                    <span className='flex items-center gap-2.5'>
                                                        <svg width="18" height="14" viewBox="0 0 18 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M14.625 3.0625H1.6875V1.375C1.6875 1.22582 1.62824 1.08274 1.52275 0.977253C1.41726 0.871763 1.27418 0.8125 1.125 0.8125C0.975816 0.8125 0.832742 0.871763 0.727252 0.977253C0.621763 1.08274 0.5625 1.22582 0.5625 1.375V12.625C0.5625 12.7742 0.621763 12.9173 0.727252 13.0227C0.832742 13.1282 0.975816 13.1875 1.125 13.1875C1.27418 13.1875 1.41726 13.1282 1.52275 13.0227C1.62824 12.9173 1.6875 12.7742 1.6875 12.625V10.375H16.3125V12.625C16.3125 12.7742 16.3718 12.9173 16.4773 13.0227C16.5827 13.1282 16.7258 13.1875 16.875 13.1875C17.0242 13.1875 17.1673 13.1282 17.2727 13.0227C17.3782 12.9173 17.4375 12.7742 17.4375 12.625V5.875C17.4375 5.12908 17.1412 4.41371 16.6137 3.88626C16.0863 3.35882 15.3709 3.0625 14.625 3.0625ZM1.6875 4.1875H6.75V9.25H1.6875V4.1875Z" fill="#344054" />
                                                        </svg>
                                                        {hotel.roomType}
                                                    </span>
                                                </div>
                                            </div>

                                            {/* Price */}
                                            <div className="text-right">
                                                <p className="!text-[1.75rem] font-semibold whitespace-nowrap">{hotel.price}</p>
                                                <p className="font-medium whitespace-nowrap">{hotel.totalPrice}</p>
                                                <p className="font-medium">{hotel.stayDetails}</p>
                                            </div>
                                        </div>

                                        {/* Facilities */}
                                        <div className="flex flex-wrap gap-4 text-lg text-TEXT-1 border-y px-4 pt-2">
                                            <span className="flex items-center gap-3 font-medium">
                                                Facilities:
                                                <span className="flex items-center gap-1">
                                                    {hotel.facilities.hasSwimming_pool && <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M5.875 10.6711C6.12364 10.6711 6.3621 10.5723 6.53791 10.3965C6.71373 10.2207 6.8125 9.98223 6.8125 9.73359V9.625H11.1875V9.88203C11.1875 10.1307 11.2863 10.3691 11.4621 10.5449C11.6379 10.7208 11.8764 10.8195 12.125 10.8195C12.3736 10.8195 12.6121 10.7208 12.7879 10.5449C12.9637 10.3691 13.0625 10.1307 13.0625 9.88203V1.5C13.0625 1.25136 12.9637 1.0129 12.7879 0.837087C12.6121 0.661272 12.3736 0.5625 12.125 0.5625C11.8764 0.5625 11.6379 0.661272 11.4621 0.837087C11.2863 1.0129 11.1875 1.25136 11.1875 1.5V2.125H6.8125V1.5C6.8125 1.25136 6.71373 1.0129 6.53791 0.837087C6.3621 0.661272 6.12364 0.5625 5.875 0.5625C5.62636 0.5625 5.3879 0.661272 5.21209 0.837087C5.03627 1.0129 4.9375 1.25136 4.9375 1.5V9.73359C4.9375 9.98223 5.03627 10.2207 5.21209 10.3965C5.3879 10.5723 5.62636 10.6711 5.875 10.6711ZM6.8125 7.75V6.8125H11.1875V7.75H6.8125ZM11.1875 4V4.9375H6.8125V4H11.1875ZM0.5625 11.8125C0.5625 11.5639 0.661272 11.3254 0.837087 11.1496C1.0129 10.9738 1.25136 10.875 1.5 10.875C2.72109 10.875 3.375 11.3086 3.89531 11.6562C4.31719 11.9375 4.59844 12.125 5.25 12.125C5.90156 12.125 6.18203 11.9391 6.60469 11.6562C7.12734 11.307 7.77656 10.875 8.99922 10.875C10.2219 10.875 10.8742 11.3086 11.3945 11.6562C11.8172 11.9375 12.0977 12.125 12.75 12.125C13.4023 12.125 13.6828 11.9391 14.1047 11.6562C14.6281 11.307 15.2766 10.875 16.5 10.875C16.7486 10.875 16.9871 10.9738 17.1629 11.1496C17.3387 11.3254 17.4375 11.5639 17.4375 11.8125C17.4375 12.0611 17.3387 12.2996 17.1629 12.4754C16.9871 12.6512 16.7486 12.75 16.5 12.75C15.8461 12.75 15.5672 12.9359 15.1445 13.2188C14.6219 13.568 13.9727 14 12.75 14C11.5273 14 10.875 13.5664 10.3547 13.2188C9.93203 12.9375 9.65156 12.75 8.99922 12.75C8.34687 12.75 8.06719 12.9359 7.64453 13.2188C7.12188 13.568 6.47266 14 5.25 14C4.02734 14 3.375 13.5664 2.85469 13.2188C2.43281 12.9375 2.15156 12.75 1.5 12.75C1.25136 12.75 1.0129 12.6512 0.837087 12.4754C0.661272 12.2996 0.5625 12.0611 0.5625 11.8125ZM17.4375 15.25C17.4375 15.4986 17.3387 15.7371 17.1629 15.9129C16.9871 16.0887 16.7486 16.1875 16.5 16.1875C15.8461 16.1875 15.5672 16.3734 15.1445 16.6562C14.6219 17.0055 13.9727 17.4375 12.75 17.4375C11.5273 17.4375 10.875 17.0039 10.3547 16.6562C9.93203 16.375 9.65156 16.1875 8.99922 16.1875C8.34687 16.1875 8.06719 16.3734 7.64453 16.6562C7.12188 17.0055 6.47266 17.4375 5.25 17.4375C4.02734 17.4375 3.375 17.0039 2.85469 16.6562C2.43281 16.375 2.15156 16.1875 1.5 16.1875C1.25136 16.1875 1.0129 16.0887 0.837087 15.9129C0.661272 15.7371 0.5625 15.4986 0.5625 15.25C0.5625 15.0014 0.661272 14.7629 0.837087 14.5871C1.0129 14.4113 1.25136 14.3125 1.5 14.3125C2.72109 14.3125 3.375 14.7461 3.89531 15.0938C4.31719 15.375 4.59844 15.5625 5.25 15.5625C5.90156 15.5625 6.18203 15.3766 6.60469 15.0938C7.12734 14.7445 7.77656 14.3125 8.99922 14.3125C10.2219 14.3125 10.8742 14.7461 11.3945 15.0938C11.8172 15.375 12.0977 15.5625 12.75 15.5625C13.4023 15.5625 13.6828 15.3766 14.1047 15.0938C14.6281 14.7445 15.2766 14.3125 16.5 14.3125C16.7486 14.3125 16.9871 14.4113 17.1629 14.5871C17.3387 14.7629 17.4375 15.0014 17.4375 15.25Z" fill="#475367" />
                                                    </svg>}
                                                    {hotel.facilities.otherfacilities && hotel.facilities.otherfacilities.join(', ')}
                                                </span>
                                            </span>
                                            {hotel.facilities.hasBar && <span className="flex items-center gap-1">
                                                <svg width="14" height="20" viewBox="0 0 14 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M13.3415 7.38594L11.64 1.61016C11.5828 1.41612 11.4645 1.24576 11.3025 1.12453C11.1406 1.0033 10.9438 0.937689 10.7415 0.9375H3.25873C3.05645 0.937689 2.85966 1.0033 2.69773 1.12453C2.53581 1.24576 2.41743 1.41612 2.36029 1.61016L0.658729 7.38594C0.375499 8.3268 0.360968 9.32811 0.616777 10.2768C0.872585 11.2255 1.38854 12.0838 2.10638 12.7547C3.19665 13.7772 4.58052 14.4314 6.06263 14.625V17.1875H3.87513C3.62649 17.1875 3.38804 17.2863 3.21222 17.4621C3.03641 17.6379 2.93763 17.8764 2.93763 18.125C2.93763 18.3736 3.03641 18.6121 3.21222 18.7879C3.38804 18.9637 3.62649 19.0625 3.87513 19.0625H10.1251C10.3738 19.0625 10.6122 18.9637 10.788 18.7879C10.9639 18.6121 11.0626 18.3736 11.0626 18.125C11.0626 17.8764 10.9639 17.6379 10.788 17.4621C10.6122 17.2863 10.3738 17.1875 10.1251 17.1875H7.93763V14.625C9.42002 14.4315 10.8042 13.7773 11.8947 12.7547C12.6124 12.0837 13.1282 11.2254 13.3839 10.2767C13.6395 9.328 13.6249 8.32673 13.3415 7.38594ZM3.95951 2.8125H10.0408L11.4767 7.68828C10.6267 7.90703 9.28529 7.92266 7.42357 6.97578C5.64701 6.07656 4.18138 5.85938 3.03451 5.95469L3.95951 2.8125ZM7.00013 12.8125C5.65768 12.8141 4.36517 12.3036 3.38607 11.3852C2.92888 10.9556 2.59935 10.408 2.43391 9.80287C2.26846 9.19775 2.27353 8.55867 2.44857 7.95625C3.8892 7.53984 5.46732 8.08594 6.57904 8.64922C7.48138 9.10625 8.93685 9.68672 10.5056 9.68672C10.8803 9.68775 11.2542 9.65243 11.622 9.58125C11.4836 10.2728 11.132 10.9037 10.6165 11.3852C9.63686 12.3042 8.3434 12.8147 7.00013 12.8125Z" fill="#475367" />
                                                </svg>
                                                Bar
                                            </span>}

                                            <div className='flex ml-auto'>
                                                <span className="flex items-center gap-2">
                                                    <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M14.25 2.1875H12.6875V1.875C12.6875 1.62636 12.5887 1.3879 12.4129 1.21209C12.2371 1.03627 11.9986 0.9375 11.75 0.9375C11.5014 0.9375 11.2629 1.03627 11.0871 1.21209C10.9113 1.3879 10.8125 1.62636 10.8125 1.875V2.1875H5.1875V1.875C5.1875 1.62636 5.08873 1.3879 4.91291 1.21209C4.7371 1.03627 4.49864 0.9375 4.25 0.9375C4.00136 0.9375 3.7629 1.03627 3.58709 1.21209C3.41127 1.3879 3.3125 1.62636 3.3125 1.875V2.1875H1.75C1.3356 2.1875 0.938171 2.35212 0.645146 2.64515C0.35212 2.93817 0.1875 3.3356 0.1875 3.75V16.25C0.1875 16.6644 0.35212 17.0618 0.645146 17.3549C0.938171 17.6479 1.3356 17.8125 1.75 17.8125H14.25C14.6644 17.8125 15.0618 17.6479 15.3549 17.3549C15.6479 17.0618 15.8125 16.6644 15.8125 16.25V3.75C15.8125 3.3356 15.6479 2.93817 15.3549 2.64515C15.0618 2.35212 14.6644 2.1875 14.25 2.1875ZM3.3125 4.0625C3.3125 4.31114 3.41127 4.5496 3.58709 4.72541C3.7629 4.90123 4.00136 5 4.25 5C4.49864 5 4.7371 4.90123 4.91291 4.72541C5.08873 4.5496 5.1875 4.31114 5.1875 4.0625H10.8125C10.8125 4.31114 10.9113 4.5496 11.0871 4.72541C11.2629 4.90123 11.5014 5 11.75 5C11.9986 5 12.2371 4.90123 12.4129 4.72541C12.5887 4.5496 12.6875 4.31114 12.6875 4.0625H13.9375V5.9375H2.0625V4.0625H3.3125ZM2.0625 15.9375V7.8125H13.9375V15.9375H2.0625Z" fill="#475367" />
                                                    </svg>
                                                    {hotel.checkIn}
                                                </span>
                                                <span className="flex items-center gap-2 ml-3">
                                                    <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M14.25 2.1875H12.6875V1.875C12.6875 1.62636 12.5887 1.3879 12.4129 1.21209C12.2371 1.03627 11.9986 0.9375 11.75 0.9375C11.5014 0.9375 11.2629 1.03627 11.0871 1.21209C10.9113 1.3879 10.8125 1.62636 10.8125 1.875V2.1875H5.1875V1.875C5.1875 1.62636 5.08873 1.3879 4.91291 1.21209C4.7371 1.03627 4.49864 0.9375 4.25 0.9375C4.00136 0.9375 3.7629 1.03627 3.58709 1.21209C3.41127 1.3879 3.3125 1.62636 3.3125 1.875V2.1875H1.75C1.3356 2.1875 0.938171 2.35212 0.645146 2.64515C0.35212 2.93817 0.1875 3.3356 0.1875 3.75V16.25C0.1875 16.6644 0.35212 17.0618 0.645146 17.3549C0.938171 17.6479 1.3356 17.8125 1.75 17.8125H14.25C14.6644 17.8125 15.0618 17.6479 15.3549 17.3549C15.6479 17.0618 15.8125 16.6644 15.8125 16.25V3.75C15.8125 3.3356 15.6479 2.93817 15.3549 2.64515C15.0618 2.35212 14.6644 2.1875 14.25 2.1875ZM3.3125 4.0625C3.3125 4.31114 3.41127 4.5496 3.58709 4.72541C3.7629 4.90123 4.00136 5 4.25 5C4.49864 5 4.7371 4.90123 4.91291 4.72541C5.08873 4.5496 5.1875 4.31114 5.1875 4.0625H10.8125C10.8125 4.31114 10.9113 4.5496 11.0871 4.72541C11.2629 4.90123 11.5014 5 11.75 5C11.9986 5 12.2371 4.90123 12.4129 4.72541C12.5887 4.5496 12.6875 4.31114 12.6875 4.0625H13.9375V5.9375H2.0625V4.0625H3.3125ZM2.0625 15.9375V7.8125H13.9375V15.9375H2.0625Z" fill="#475367" />
                                                    </svg>
                                                    {hotel.checkOut}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <button onClick={() => { addHotel(hotel); toast.success("New Hotel added to your itinerary") }} className='!w-12 bg-green-400 flex justify-center items-center'>
                                <svg width="20" height="20" fill="white" xmlns="http://www.w3.org/2000/svg" className="" viewBox="0 0 512 512"><path d="M256 48C141.31 48 48 141.31 48 256s93.31 208 208 208 208-93.31 208-208S370.69 48 256 48zm80 224h-64v64a16 16 0 01-32 0v-64h-64a16 16 0 010-32h64v-64a16 16 0 0132 0v64h64a16 16 0 010 32z" /></svg>
                            </button>
                        </div>
                    ))}
                </div>}
            </div>
        </div >
    )
}